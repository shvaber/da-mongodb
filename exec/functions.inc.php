...
define("PLUGIN_NAME",        "MongoDB Manager for Directadmin");
define("PLUGIN_VERSION",     "0.2.2");
define("PLUGIN_DIR",         "/usr/local/directadmin/plugins/mongodb");
define("PLUGIN_CONF_FILE",   "/usr/local/directadmin/plugins/mongodb/mongopass.conf");
define("PLUGIN_IMAGES_DIR",  "/usr/local/directadmin/plugins/mongodb/images");
define("PLUGIN_DATA_DIR",    "/usr/local/directadmin/plugins/mongodb/data");
define("PLUGIN_JS_DIR",      "/usr/local/directadmin/plugins/mongodb/data/_js");
define("PLUGIN_CSS_DIR",     "/usr/local/directadmin/plugins/mongodb/data/_css");
define("PLUGIN_TPL_DIR",     "/usr/local/directadmin/plugins/mongodb/data/_tpl");
define("PLUGIN_SSO_DIR",     "/usr/local/directadmin/plugins/mongodb/data/sso");
define("PLUGIN_EXEC_DIR",    "/usr/local/directadmin/plugins/mongodb/exec");
define("PLUGIN_ACTION_DIR",  "/usr/local/directadmin/plugins/mongodb/exec/actions");
define("PLUGIN_TOOLS_DIR",   "/usr/local/directadmin/plugins/mongodb/exec/tools");
define("PLUGIN_LANG_DIR",    "/usr/local/directadmin/plugins/mongodb/lang");
define("PLUGIN_LOGS_DIR",    "/usr/local/directadmin/plugins/mongodb/logs");
define("PLUGIN_UPLOAD_DIR",  "/home/tmp/mongodb_restore");

define("PLUGIN_MOVE_BIN",    "/usr/local/directadmin/plugins/mongodb/exec/move_uploaded_file");
define("PLUGIN_RESTORE_BIN", "/usr/local/directadmin/plugins/mongodb/exec/dbrestore.sh");
define("PLUGIN_DUMPDB_BIN", "/usr/local/directadmin/plugins/mongodb/exec/dbdumpsingle.sh");

define("PLUGIN_TPL_MAIN",    "/usr/local/directadmin/plugins/mongodb/data/_tpl/main.html");
define("PLUGIN_TPL_ERROR",   "/usr/local/directadmin/plugins/mongodb/data/_tpl/error.html");
define("PLUGIN_CSS_FILE",    "/usr/local/directadmin/plugins/mongodb/data/_css/plugins.css");
define("PLUGIN_JS_FILE",     "/usr/local/directadmin/plugins/mongodb/data/_js/plugins.js");
define("PLUGIN_JS_ADMIN_FILE","/usr/local/directadmin/plugins/mongodb/data/_js/admin/plugins.js");

define("MONGOADMIN_LINK",    "#");
...
function generate_page($data)
{
    ...
    {
        $data['PLUGIN_BASE_URL'] = (isset($_SERVER["REQUEST_URI"]) && (strpos($_SERVER["REQUEST_URI"], "CMD_PLUGINS_ADMIN") === false)) ? "/CMD_PLUGINS/mongodb" : "/CMD_PLUGINS_ADMIN/mongodb";
        $data['CSS_PLUGIN_CODE'] = _get_css(PLUGIN_CSS_FILE);
        $data['JS_PLUGIN_CODE'] = (defined('PLUGIN_IS_ADMIN') && PLUGIN_IS_ADMIN) ? _get_js(PLUGIN_JS_ADMIN_FILE) : _get_js(PLUGIN_JS_FILE);
...
{
    if (defined('PLUGIN_CONF_FILE') && PLUGIN_CONF_FILE && is_file(PLUGIN_CONF_FILE))
    {
        $conf = _get_user_credentials(PLUGIN_CONF_FILE);
        if (isset($conf['dbhost']) && $conf['dbhost']) define('MONGO_HOST', $conf['dbhost']); else define('MONGO_HOST', false);
        if (isset($conf['dbport']) && $conf['dbport']) define('MONGO_PORT', $conf['dbport']); else define('MONGO_PORT', false);
        if (isset($conf['dbname']) && $conf['dbname']) define('MONGO_DB', $conf['dbname']); else define('MONGO_DB', false);
        if (isset($conf['dbuser']) && $conf['dbuser']) define('MONGO_USER', $conf['dbuser']); else define('MONGO_USER', false);
        if (isset($conf['dbpass']) && $conf['dbpass']) define('MONGO_PASSWORD', $conf['dbpass']); else define('MONGO_PASSWORD', false);
        return true;
    }
    return false;
}

function _get_user_credentials($file)
{
    $return = false;
...
function _save_user_credentials($file, $input)
{
