...
if ($mongodb_user_databases = $mongodb->getDatabasesList($USER))
{
    ...
    if (in_array($database, $databases))
    {
        ...
        if ($mongodb_all_privileges = $mongodb->getPrivilegesList($database))
        {
            foreach ($mongodb_all_privileges as $key => $row)
            {
                if (($key = array_search($row['user'], $mongodb_db_users)) !== false)
                {
                    unset($mongodb_db_users[$key]);
                }
                else if (($USER !== $row['user']) && (strpos($row['user'], $USER."_")!==0))
                {
                    $mongodb_all_privileges[$key]['id'] = false;
                    $mongodb_all_privileges[$key]['password'] = false;
                    $mongodb_all_privileges[$key]['privileges'] = false;
                }
            }
            reset($mongodb_all_privileges);
            usort($mongodb_all_privileges, "sort_privileges");
            $TABLE_LIST = format_table_list($mongodb_all_privileges, 'table_list', [
                        ...
                        'password'   => '<a href="/CMD_PLUGINS/mongodb/database.html?dbname='.$database.'&mode=password&dbuser=|VAL_USER|" class="text-dark px_plugin_change_password" data-toggle="modal" data-target="#px_ModalCenter">'.$da->get_lang('PLUGIN_CHANGE_PASSWORD').'</a>',
        ...
        sort($mongodb_db_users);
        array_unshift($mongodb_db_users, '');
    }
    else
    {
        $is_error = true;
...
$MAIN_CONTENT = [
    ...
    'PLUGIN_SELECT_DB_USER'       => ($mongodb_db_users) ? array2options($mongodb_db_users, false) : '',
    'DISABLED'                    => '',
];
