...
PLUGIN_DIR="/usr/local/directadmin/plugins/mongodb";
...
set_permissions diradmin:diradmin 750 "${PLUGIN_DIR}/exec";
set_permissions diradmin:diradmin 750 "${PLUGIN_DIR}/exec/dbbackupuser.sh";
set_permissions diradmin:diradmin 750 "${PLUGIN_DIR}/exec/dbdumpall.sh";
set_permissions diradmin:diradmin 750 "${PLUGIN_DIR}/exec/dbdumpsingle.sh";
set_permissions diradmin:diradmin 750 "${PLUGIN_DIR}/exec/dbsize.sh";
...
set_permissions diradmin:diradmin 750 "${PLUGIN_DIR}/hooks";
set_permissions diradmin:diradmin 750 "${PLUGIN_DIR}/hooks/other_disk_usage.sh";
set_permissions diradmin:diradmin 750 "${PLUGIN_DIR}/hooks/user_backup_compress_pre.sh";
set_permissions diradmin:diradmin 750 "${PLUGIN_DIR}/hooks/user_restore_post.sh";
set_permissions diradmin:diradmin 750 "${PLUGIN_DIR}/scripts/setup";
...
CPIL="mongodb=type=text&string=MongoDB Databases&desc=Allow to create MongoDB databases&default=5";

if [ -f "${CPIF}" ];
then
    INSTALL_CPIF=0;
    c=$(grep -m1 -c "^mongodb=" "${CPIF}");
    if [ "${c}" -eq "0" ];
    then
        INSTALL_CPIF=1;
    fi;
